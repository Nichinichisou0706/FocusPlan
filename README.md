...
脚本会运行单元测试、Lint 和 Debug 构建，输出位置为 `dist/FocusPlan-0.1.0-debug.apk`。`dist` 不纳入 Git，发布包见 GitHub Releases。
